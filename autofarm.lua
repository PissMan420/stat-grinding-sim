...
-- Find the parant of a instance. The parent mean here that its a deeply nested parent so its not directly the parent.
-- This work by finding the first parent that is not a part or a model
local function findParent(instance)
...
-- find out if the part is a portal. 
-- a instance is a portal if one of the parents and grand father is named "Portals" and 
-- is a folder.
local function isPortal(instance)
  local parent = instance.Parent
  while parent do
    if parent.Name == "Portals" and parent:IsA("Folder") then
      return true
    end
    parent = parent.Parent
  end
...
-- touch a part for a ms or so just so it pretend to be touched using firetouchinterest
-- firetouchinterest is a function that is used to simulate a touch on a part
local function touchPart(touchinterest)
  local hrp = players.LocalPlayer.Character.HumanoidRootPart
  firetouchinterest(touchinterest,hrp, 1)
  -- wait the time it would ussally take to touch the part and then leave the part
  wait(0.1)
  firetouchinterest(touchinterest,hrp, 0)
end

local touchInterest = {}
-- find all the touch interests
for k, v in next, workspace:GetDescendants() do
  if v:IsA("TouchInterest") and not isPortal(v) then
    touchInterest[#touchInterest + 1] = v
  end
end
...
local win = library:CreateWindow({ Name = "Grinding Simulator autofarm", Themeable = true })

-- Create a tab for the Auto farm (everything will be added on this tab)
local autofarmTab = win:CreateTab({ Name = "Auto Farm Settings" })

-- Option section 
-- It should have a option to set the number of threads to use
-- It should have a label. The label should have a message telling the user about the danger of using a lot of threads with emojis.
local optionsSection = autofarmTab:CreateSection({ Name = "Auto Farm Settings" })

local threadOption = optionsSection:AddTextbox({
  Name = "Threads",
  Value = opts.threadCount,
...
optionsSection:AddLabel({
  Text = "Warning: Using a lot of threads"
})
optionsSection:AddLabel({
  Text = "can cause lag and/or crash."
})
optionsSection:AddLabel({
  Text = "Use with caution."
})

-- Create a section for starting and turning off the auto farm
-- It should have a toggle to start and stop the auto farm
local autoFarmSection = autofarmTab:CreateSection({ Name = "Auto Farm" })
autoFarmSection:AddToggle({
  Name = "Run autofarm?", 
  Callback = function(value)
    opts.isEnabled = value
    print("Auto Farm is " .. (value and "enabled" or "disabled"))
  end,
  Value = opts.isEnabled
})

game:GetService("RunService").Stepped:Connect(function()
  if opts.isEnabled then
    -- run the auto farm
    for i = 1, opts.threadCount do
      -- run the auto farm in a thread
      spawn(function()
        -- touch all the fucking parts
        for k, v in ipairs(touchInterest) do
          touchPart(v)
        end
      end)
    end
